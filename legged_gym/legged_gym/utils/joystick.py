...
def key_response_vel(
    ...
    if key.char == "w":
        ...
        env.command_ranges["lin_vel_x"] = [
            command_state["vel_forward"],
            command_state["vel_forward"],
        ]
        env._resample_commands([0])
        print(
            f"{key.char} pressed, forward speed: ", command_state["vel_forward"]
        )
        ...
        env.command_ranges["lin_vel_y"] = [
            command_state["vel_side"],
            command_state["vel_side"],
        ]
        env._resample_commands([0])
        ...
        env.command_ranges["ang_vel_yaw"] = [
            command_state["orientation"],
            command_state["orientation"],
        ]
        env._resample_commands([0])
        print(
            f"{key.char} pressed, face orientation: ",
            command_state["orientation"],
        )
    elif key.char == "e":
        if command_state["orientation"] > -2.0:
...
def key_response_joint_monkey(
    ...
    if key.char == "[":
        ...
        env.command_ranges["torso_angle"] = [
            command_state["torso"],
            command_state["torso"],
        ]
        env._resample_commands([0])
        print(
            f"{key.char} pressed, torso orientation: ", command_state["torso"]
        )
    elif key.char == "]":
        if command_state["torso"] > -2.5:
            command_state["torso"] -= 0.5
        ...
        env.command_ranges["left_elbow_angle"] = [
            command_state["left_elbow"],
            command_state["left_elbow"],
        ]
        env._resample_commands([0])
        print(
            f"{key.char} pressed, left elbow orientation: ",
            command_state["left_elbow"],
        )
    elif key.char == "<":
        ...
        env.command_ranges["right_elbow_angle"] = [
            command_state["right_elbow"],
            command_state["right_elbow"],
        ]
        env._resample_commands([0])
        print(
            f"{key.char} pressed, torso orientation: ",
            command_state["right_elbow"],
        )
    elif key.char == ">":
    ...
    # 'n' for left shoulder yaw increase, 'N' for decrease, range [-4.45, 1.3]
        ...
        env.command_ranges["left_shoulder_yaw_angle"] = [
            command_state["left_shoulder_yaw"],
            command_state["left_shoulder_yaw"],
        ]
        env._resample_commands([0])
        print(
            f"{key.char} pressed, left_shoulder_yaw: ",
            command_state["left_shoulder_yaw"],
        )
    elif key.char == "N":
    ...
    # 'm' for right shoulder yaw increase, 'M' for decrease, range [-1.3, 4.45]
        ...
        env.command_ranges["right_shoulder_yaw_angle"] = [
            command_state["right_shoulder_yaw"],
            command_state["right_shoulder_yaw"],
        ]
        env._resample_commands([0])
        print(
            f"{key.char} pressed, right_shoulder_yaw: ",
            command_state["right_shoulder_yaw"],
        )
    elif key.char == "M":
    ...
    # 'h' for left shoulder pitch increase, 'H' for decrease, range [-2.87, 2.87]
        ...
        env.command_ranges["left_shoulder_pitch_angle"] = [
            command_state["left_shoulder_pitch"],
            command_state["left_shoulder_pitch"],
        ]
        env._resample_commands([0])
        print(
            f"{key.char} pressed, left_shoulder_pitch: ",
            command_state["left_shoulder_pitch"],
        )
    elif key.char == "H":
        if command_state["left_shoulder_pitch"] > -2.87:
    ...
    # 'j' for right shoulder pitch increase, 'J' for decrease, range [-2.87, 2.87]
        ...
        env.command_ranges["right_shoulder_pitch_angle"] = [
            command_state["right_shoulder_pitch"],
            command_state["right_shoulder_pitch"],
        ]
        env._resample_commands([0])
        print(
            f"{key.char} pressed, right_shoulder_pitch: ",
            command_state["right_shoulder_pitch"],
        )
    elif key.char == "J":
        if command_state["right_shoulder_pitch"] > -2.87:
    ...
    # 'y' for left shoulder roll increase, 'Y' for decrease, range [-3.11, 0.34]
        ...
        env.command_ranges["left_shoulder_roll_angle"] = [
            command_state["left_shoulder_roll"],
            command_state["left_shoulder_roll"],
        ]
        env._resample_commands([0])
        print(
            f"{key.char} pressed, left_shoulder_roll: ",
            command_state["left_shoulder_roll"],
        )
    elif key.char == "Y":
    ...
    # 'u' for right shoulder roll increase, 'U' for decrease, range [-3.11, 0.34]
        ...
        env.command_ranges["right_shoulder_roll_angle"] = [
            command_state["right_shoulder_roll"],
            command_state["right_shoulder_roll"],
        ]
        env._resample_commands([0])
        print(
            f"{key.char} pressed, right_shoulder_roll: ",
            command_state["right_shoulder_roll"],
        )
    elif key.char == "U":
