...
class LeggedRobot(BaseTask):
    ...
    def post_physics_step(self):
        ...
        self.joint_hist[:, 0, 0 : self.num_dof] = (
            self.dof_pos.clone() - self.dof_bias
        )
        self.joint_hist[:, 0, self.num_dof : 2 * self.num_dof] = (
            self.dof_vel.clone()
        )
        self.joint_hist[:, 0, 2 * self.num_dof : 3 * self.num_dof] = (
            self.actions.clone()
        )
    ...
    def check_termination(self):
        ...
        if (
            self.cfg.asset.terminate_by_feet_distance
            and self.feet_distance.shape
        ):
            self.reset_buf |= torch.any(
                self.feet_distance
                < self.cfg.asset.termination_scales.min_feet_distance,
                dim=1,
            )
        ...
        if self.cfg.asset.terminate_by_xy:
            self.reset_buf |= (
                torch.abs(self.root_states[:, 0] - self.env_origins[:, 0])
                > self.cfg.asset.termination_scales.global_xy
            )
            self.reset_buf |= (
                torch.abs(self.root_states[:, 1] - self.env_origins[:, 1])
                > self.cfg.asset.termination_scales.global_xy
            )
        ...
        if self.cfg.asset.terminate_by_hip_yaw:
            self.reset_buf |= (
                torch.abs(self.dof_pos[:, 0] - self.dof_pos[:, 5])
                > self.cfg.asset.termination_scales.hip_yaw_sum
            )
    ...
    def reset_idx(self, env_ids):
        ...
        if self.cfg.control.action_filt:
            ...
            filter_action_ids_torch = torch.concat(
                [
                    torch.arange(
                        self.num_actions, dtype=torch.int64, device=self.device
                    )
        ...
        if self.cfg.domain_rand.randomize_dof_bias:
            self.dof_bias[env_ids] = self.dof_bias[env_ids].uniform_(
                -self.cfg.domain_rand.max_dof_bias,
                self.cfg.domain_rand.max_dof_bias,
            )
        ...
        self.joint_hist[env_ids, :, 0 : self.num_dof] = (
            self.dof_pos[env_ids] - self.dof_bias[env_ids]
        ).unsqueeze(1)
        self.joint_hist[env_ids, :, self.num_dof : 2 * self.num_dof] = 0.0
        self.joint_hist[env_ids, :, 2 * self.num_dof : 3 * self.num_dof] = (
            self.dof_pos[env_ids]
            - self.default_dof_pos
            - self.dof_bias[env_ids]
        ).unsqueeze(1) / self.cfg.control.action_scale
    ...
    def compute_observations(self):
        ...
        self.obs_buf = torch.cat(
            (
                ...
                (self.dof_pos - self.default_dof_pos - self.dof_bias)
    ...
    def _process_rigid_shape_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_friction:
            ...
            for s in range(len(self._ground_friction_values[0])):
                # breakpoint()
                if (
                    s < len(self._ground_friction_values[0]) - 1
                ):  # temperory fix for gpr, not for h1
        ...
        if env_id == 0:
            ...
            for i in range(len(props)):
                ...
                self.torque_limits[i] = (
                    props["effort"][i].item()
                    * self.cfg.control.torque_effort_scale
                )
                # soft limits
    ...
    def _reset_dofs(self, env_ids):
        ...
        self.dof_pos[env_ids] = self.default_dof_pos + torch_rand_float(
            -0.4, 0.4, (len(env_ids), self.num_dof), device=self.device
        )
    ...
    def _push_robots(self):
        ...
        self.gym.set_actor_root_state_tensor(
            self.sim, gymtorch.unwrap_tensor(self.root_states_all)
        )
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[9 + 2 * self.num_actions :] = 0.0  # previous actions
        return noise_vec
    ...
    def _init_buffers(self):
        ...
        self.joint_hist = torch.zeros(
            self.num_envs,
            self.cfg.env.joint_hist_len,
            self.num_dof * 3,
            device=self.device,
        )  # q, dq, joint action
    ...
    def _get_env_origins(self):
        ...
        if self.cfg.terrain.mesh_type in ["trimesh"]:
            ...
            self.terrain_box_upper_vetices = (
                torch.from_numpy(self.terrain.box_upper_vetices)
                .to(self.device)
                .to(torch.float)
            )
            self.box_upper_vertices = self.terrain_box_upper_vetices[
                self.terrain_levels, self.terrain_types
            ].clone()

            # self.terrain_motion_pool = torch.from_numpy(self.terrain.motion_pool).to(self.device).to(torch.long)
            self.terrain_sequence_pool_size = (
                torch.from_numpy(self.terrain.sequence_pool_size)
                .to(self.device)
                .to(torch.long)
            )
            # self.motion_pool = self.terrain_motion_pool[self.terrain_levels, self.terrain_types].clone()
            self.sequence_pool_size = self.terrain_sequence_pool_size[
                self.terrain_levels, self.terrain_types
            ].clone()
            # import pdb; pdb.set_trace()
    ...
    def _resample_motions(self, env_ids):
        ...
        self.env_origins_init_3Doffset[env_ids, :2] = torch_rand_float(
            -1.0, 1.0, (len(env_ids), 2), device=self.device
        )  # xy position within 1m of the center
        self.motion_times[env_ids] = self._motion_lib.sample_time(
            self.motion_ids[env_ids]
        )
        motion_res = self._motion_lib.get_motion_state(
            self.motion_ids[env_ids], self.motion_times[env_ids]
        )
        self.ref_base_pos_init[env_ids] = (
            motion_res["root_pos"]
            + self.env_origins[env_ids]
            + self.env_origins_init_3Doffset[env_ids]
        )  # ZL: fix later. Ugly code.
    ...
    def get_ee_pos_b(self):
        ...
        left_foot_pos_b = quat_rotate_inverse(
            self.base_quat, left_foot_pos - self.base_pos
        )
        right_foot_pos_b = quat_rotate_inverse(
            self.base_quat, right_foot_pos - self.base_pos
        )
        left_hand_pos_b = quat_rotate_inverse(
            self.base_quat, left_hand_pos - self.base_pos
        )
        right_hand_pos_b = quat_rotate_inverse(
            self.base_quat, right_hand_pos - self.base_pos
        )
        return (
            left_foot_pos_b,
            right_foot_pos_b,
    ...
    def get_ee_pos_yawb(self):
        ...
        left_foot_pos_b = quat_rotate_inverse(
            yawquat, left_foot_pos - self.base_pos
        )
        right_foot_pos_b = quat_rotate_inverse(
            yawquat, right_foot_pos - self.base_pos
        )
        left_hand_pos_b = quat_rotate_inverse(
            yawquat, left_hand_pos - self.base_pos
        )
        right_hand_pos_b = quat_rotate_inverse(
            yawquat, right_hand_pos - self.base_pos
        )
        return (
            left_foot_pos_b,
            right_foot_pos_b,
    ...
    def hand_state(self):
        ...
        left_state = torch.cat(
            [left_hand_pos, left_hand_ori], dim=-1
        ).unsqueeze(1)
        right_state = torch.cat(
            [right_hand_pos, right_hand_ori], dim=-1
        ).unsqueeze(1)
    ...
    def _reward_torque_limits(self):
        ...
        return torch.sum(
            (
                ...
                - self.cfg.rewards.soft_torque_limit
            ).clip(min=0.0),
            dim=1,
        )
    ...
    def _reward_stumble(self):
        ...
        return torch.any(
            ...
            > 2 * torch.abs(self.contact_forces[:, self.feet_indices, 2]),
            dim=1,
        )
    ...
    def _reward_vel_x(self):
        return 1.0 * torch.abs(self.base_lin_vel[:, 0])

    def _reward_hand_vel_w(self):
        return self._rigid_body_vel[:, self.hand_indices[0] - 1, :].norm(
            dim=-1
        ) + self._rigid_body_vel[:, self.hand_indices[1] - 1, :].norm(dim=-1)
