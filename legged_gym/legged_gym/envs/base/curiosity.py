    ...
    def __init__(self, input_size, hidden_sizes, output_size):
        ...
        for layer_index in range(len(hidden_sizes)):
            if layer_index == len(hidden_sizes) - 1:
                ...
                layers.append(
                    nn.Linear(
                        hidden_sizes[layer_index], hidden_sizes[layer_index + 1]
                    )
                )
                layers.append(nn.ReLU())
...
class NHashCuriosity:
    def __init__(self, cfg, device):
        ...
        self.device = device
        self.hashnn = myNN(
            cfg.curiosity.obs_dim * 2,
            cfg.curiosity.hidden_sizes_hash,
            cfg.curiosity.pred_dim,
        ).to(device)
        self.bin_cnt = torch.zeros(
            2**cfg.curiosity.pred_dim,
            dtype=torch.long,
            device=self.device,
            requires_grad=False,
        )
    ...
    def bin2int(self, bins):
        ...
        base_ = 2 ** torch.arange(
            end=self.cfg.curiosity.pred_dim, device=self.device
        ).unsqueeze(0)
    ...
    def normalize(self, obs):
        ...
        obs_ = (
            (obs_ - self.obs_lb) / (self.obs_ub - self.obs_lb) * math.pi
        )  # only half arc
    ...
    def sym_map(self, obs):
        ...
        obs2[:, 0] = obs[:, 0] * 1.0
        obs2[:, 1] = obs[:, 1] * (-1.0)
        obs2[:, 2] = obs[:, 2] * 1.0  # xyz
        obs2[:, 3] = obs[:, 3] * (-1.0)
        obs2[:, 4] = obs[:, 4] * 1.0
        obs2[:, 5] = obs[:, 5] * (-1.0)
        obs2[:, 6] = obs[:, 6] * 1.0  # quat
        obs2[:, 7:13] = obs[:, 7:13] * torch.Tensor([[1, -1, 1, -1, 1, -1]]).to(
            obs.device
        )  # twist
        obs2[:, 13:16] = obs[:, 16:19] * torch.Tensor([[1, -1, 1]]).to(
            obs.device
        )
        obs2[:, 16:19] = obs[:, 13:16] * torch.Tensor([[1, -1, 1]]).to(
            obs.device
        )
        obs2[:, 19:22] = obs[:, 22:25] * torch.Tensor([[1, -1, 1]]).to(
            obs.device
        )
        obs2[:, 22:25] = obs[:, 19:22] * torch.Tensor([[1, -1, 1]]).to(
            obs.device
        )  # ee
        obs2[:, 25] = obs[:, 26] * 1.0
        obs2[:, 26] = obs[:, 25] * 1.0
        obs2[:, 27] = obs[:, 28] * 1.0
        obs2[:, 28] = obs[:, 27] * 1.0  # ee contact
        return obs2

    def update_curiosity(self, obs: torch.Tensor, shift=0.0) -> torch.Tensor:
...
class RNDCuriosity:
    def __init__(self, cfg, device):
        ...
        self.rnd_pred = RNDNN(
            cfg.curiosity.obs_dim,
            cfg.curiosity.hidden_sizes_pred,
            cfg.curiosity.pred_dim,
        ).to(device)
        self.rnd_target = RNDNN(
            cfg.curiosity.obs_dim,
            cfg.curiosity.hidden_sizes_target,
            cfg.curiosity.pred_dim,
        ).to(device)
        for param in self.rnd_target.parameters():
            param.requires_grad = False
        self.optimizer = torch.optim.SGD(
            self.rnd_pred.parameters(), lr=cfg.curiosity.lr
        )
...
class DualDiscreteCuriosity:
    ...
    def init_buffer(
        ...
        self.curiosity_items = [
            attr
            for attr in dir(self.cfg.curiosity)
        ...
        for i in range(0, len(self.curiosity_items), 2):
            ...
            curiosity_bincnt_xy = torch.zeros(
                n_xbin,
                n_ybin,
                device=self.device,
                dtype=torch.float,
            ...
            setattr(
                self,
                f"{self.curiosity_items[i]}_{self.curiosity_items[i+1]}",
                curiosity_bincnt_xy,
            )
    ...
    def update_curiosity(self, curiosity_dict):
        ...
        for i in range(0, len(self.curiosity_items), 2):
            ...
            xls_ = torch.div(
                torch.clip(obs_x, min=xbinset[0], max=xbinset[1]) - xbinset[0],
                xbinset[2],
                rounding_mode="floor",
            ).long()
            yls_ = torch.div(
                torch.clip(obs_y, min=ybinset[0], max=ybinset[1]) - ybinset[0],
                ybinset[2],
                rounding_mode="floor",
            ).long()
            ...
            curiosity_bincnt_xy = getattr(
                self, f"{self.curiosity_items[i]}_{self.curiosity_items[i+1]}"
            )
            newbin_xyl = torch.zeros_like(curiosity_bincnt_xy).repeat(
                self.num_envs, 1, 1
            )
            newbin_xyl[
                torch.arange(self.num_envs, device=self.device), xls_, yls_
            ] += 1
