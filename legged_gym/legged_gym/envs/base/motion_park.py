...
class BaseDino:
    def __init__(
        self, left_pos: List, left_quat: List, right_pos: List, right_quat: List
    ):
...
class JurassicPark:
    ...
    def __init__(
        self, num_envs: int, num_steps_: int, motion_preset: str, device: str
    ):
        """
        Args:
        ...
        print(
            colored(
                "[Warning]: We overwrite num_steps to 1, since we only support 1 step for now",
                "red",
            )
        )
        ...
        # move to torch
        self.left_pos_pool = torch.tensor(
            left_pos_pool, device=device, dtype=torch.float
        )
        self.right_pos_pool = torch.tensor(
            right_pos_pool, device=device, dtype=torch.float
        )
        self.left_quat_pool = torch.tensor(
            left_quat_pool, device=device, dtype=torch.float
        )
        self.right_quat_pool = torch.tensor(
            right_quat_pool, device=device, dtype=torch.float
        )

        self.pool_size = torch.ones(
            (num_envs, num_steps), device=device, dtype=torch.long
        ) * len(self.motion_pool)

        self.left_pos = torch.zeros(
            (num_envs, 3, num_steps), device=device, dtype=torch.float
        )
        self.right_pos = torch.zeros(
            (num_envs, 3, num_steps), device=device, dtype=torch.float
        )
        self.left_quat = torch.zeros(
            (num_envs, 4, num_steps), device=device, dtype=torch.float
        )
        self.right_quat = torch.zeros(
            (num_envs, 4, num_steps), device=device, dtype=torch.float
        )
    ...
    def resample_motion(self, env_ids):
        if len(env_ids) <= 0:
            return
        # print("here")
        sequence_id = batch_rand_int(self.pool_size[env_ids]).to(
            device=self.device
        )
        # import pdb; pdb.set_trace()
        self.left_pos[env_ids, :, :] = self.left_pos_pool[
            sequence_id
        ].transpose(1, 2)
        self.right_pos[env_ids, :, :] = self.right_pos_pool[
            sequence_id
        ].transpose(1, 2)
        self.left_quat[env_ids, :, :] = self.left_quat_pool[
            sequence_id
        ].transpose(1, 2)
        self.right_quat[env_ids, :, :] = self.right_quat_pool[
            sequence_id
        ].transpose(1, 2)
    ...
    def get_ref_motion_obs(self, step_id):
        ...
        return torch.concat(
            (
                self.left_pos[:, :, step_id],
                self.left_quat[:, :, step_id],
                self.right_pos[:, :, step_id],
                self.right_quat[:, :, step_id],
            ),
            dim=1,
        )
